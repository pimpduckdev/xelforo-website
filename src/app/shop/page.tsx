    ...
    <div className="relative flex flex-col bg-[#0a0a0f]">
      ...
      <section className="relative flex flex-col items-center justify-center py-24 px-6 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-4xl">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">
            Xelforo <span className="text-[#4a9eff]">Shop</span>
          </h1>
      ...
      <section className="py-16 px-6 bg-[#08080c]">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories.map((cat) => (
              <Link key={cat.name} href={cat.href} className="group bg-[#111] border border-[#4a9eff]/10 rounded-xl p-5 text-center hover:border-[#4a9eff]/30 hover:bg-[#4a9eff]/5 transition-all duration-500">
                <div className="text-3xl mb-2">{cat.icon}</div>
                <div className="text-sm font-medium text-white group-hover:text-[#4a9eff] transition-colors">{cat.name}</div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {featuredProducts.map((product) => (
              <div key={product.name} className="group bg-[#111] border border-[#4a9eff]/10 rounded-xl overflow-hidden hover:border-[#4a9eff]/30 transition-all duration-500 hover:-translate-y-1">
                {/* Image placeholder */}
                <div className="h-40 bg-[#0a0a0a] flex items-center justify-center text-5xl relative">
                  ...
                  {product.badge && (
                    <span className={`absolute top-3 right-3 px-2 py-0.5 text-xs font-medium rounded-full ${
                      ...
                      product.badge === 'New' ? 'bg-[#4a9eff]/20 text-[#4a9eff] border border-[#4a9eff]/30' :
                ...
                <div className="p-4">
                  <h3 className="text-sm font-medium text-white mb-2 group-hover:text-[#4a9eff] transition-colors line-clamp-2">{product.name}</h3>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[#4a9eff] font-bold">${product.price}</span>
                  ...
                  <button className="w-full px-4 py-2 bg-[#4a9eff]/10 hover:bg-[#4a9eff]/20 border border-[#4a9eff]/20 hover:border-[#4a9eff]/40 text-[#4a9eff] text-sm font-medium rounded-lg transition-all duration-300">
                    Add to Cart
                  </button>
                </div>
              </div>
      ...
      <section className="py-16 px-6 bg-[#08080c]">
        <div className="max-w-4xl mx-auto">
          <div className="bg-[#111] border border-[#4a9eff]/15 rounded-2xl p-8 md:p-12 text-center">
